...
- **Android APK**: double-click `scripts/编译APK.bat` (syncs source to `www/` via `scripts/sync-www.js`, then builds with Gradle).
...
- Scroll mode renders a moving window of chapters (`SCROLL_WINDOW_MAX=8`); `trimScrollWindow` removes only sections outside the viewport ± buffer.
